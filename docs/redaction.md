...
图片除了 25 MB 编码大小限制外，还受两级像素预算控制。超过 `MAX_IMAGE_PIXELS`（默认 12,000,000 像素）的图片会生成按比例缩小的内部处理副本，原始文件保持不变；超过 `MAX_IMAGE_DECODE_PIXELS`（默认 25,000,000 像素）的异常大图才会被拒绝。这样可兼容 4032×3024 等常见手机照片，同时在进入 OCR/LaMa 前阻止超大输入突破 Worker 内存预算。

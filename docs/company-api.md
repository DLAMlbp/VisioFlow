...
正式模式固定执行“AI 在全部启用标准中唯一分类 -> 仅执行该标准的对应过滤规则 -> 单图通过后立即美化 -> OpenCLIP 图片向量与大模型内容特征并行生成 -> 素材库匹配 -> 继承素材组人工标签”。大模型内容特征只用于候选比对，不会直接生成正式标签。新任务只需传入 `beautify_profile`；服务端会冻结全部启用过滤标准，并要求其中恰好一条为兜底分类。`processing_standards`、`max_selected` 和历史路由字段仅兼容旧客户端，新任务会忽略。
